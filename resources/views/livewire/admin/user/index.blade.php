<div>
    ...
    @foreach ($csf_search_forms as $form_tree)
        @foreach ($form_tree as $row)
            @foreach ($row as $option)
                ...
                @if ($option['html_tag'] === 'select')
                    <select name="" id="" wire:model.change="csf_search_forms.{{ $loop->parent->parent->index }}.{{ $loop->parent->index }}.{{ $loop->index }}.obj_idx" wire:key="csf_search_forms.{{ $loop->parent->parent->index }}.{{ $loop->parent->index }}.{{ $loop->index }}.obj_idx">
                        @foreach ($option['objs'] as $item)
                            <option value="{{ $loop->index }}">{{ $item['label'] }}</option>
                        @endforeach
                    </select>
                @elseif ($option['html_tag'] === 'input')
                    <input type="text" wire:model.change="csf_search_forms.{{ $loop->parent->parent->index }}.{{ $loop->parent->index }}.{{ $loop->index }}.objs.{{ $csf_search_forms[$loop->parent->parent->index][$loop->parent->index][$loop->index]['obj_idx'] }}.data.value" placeholder="{{ $option['objs'][$option['obj_idx']]['label'] }}">
                @endif
        ...
        @if ($loop->last && count($csf_search_forms) < $csf_max_search_form_cnt)
            <button wire:click="csf_AddSearchForm">検索カラムの追加</button>
        @endif
    @endforeach
